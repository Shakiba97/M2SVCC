"""
Reproduce fig_asymmetric_altair.png from Concurrent-Asymmetric results.
Changes vs original:
  - Ped level labels: Low Ped -> Low Pedestrian Demand, etc.
  - M@SVCC header: "M@SVCC (Vehicle/Pedestrian weighting factor)"
"""
...
RESULTS_DIR = os.path.join(os.path.dirname(__file__), "Results", "Concurrent-Asymmetric")
...
# Layout constants — match the original visual style
CHART_WIDTH = 680
CELL_HEIGHT = 240   # height per ped-level section (6 metrics → 40px/cell, no text overlap)
LEFT_COL_W  = 80    # px at scale=1 for category labels on left

# Category groupings: (label, first_metric_row, last_metric_row)  — 0-indexed
CATEGORIES = [
    ("Sustainability", 0, 0),
    ("Mobility",       1, 4),
    ("Safety",         5, 5),
]
# Color: exact 5-stop RdBu-style matching the original figure's color bar
COLOR_SCHEME = None  # use custom hex range
...
def load_seed_vals(ped, weight_str, is_actuated=False):
    vals = {m: [] for m in METRIC_PATTERNS}
    for seed in SEEDS:
        if is_actuated:
            fname    = f"actuated_penetration(1)_EVratio(0)_{ped}_seed({seed}).txt"
            patterns = ACTUATED_PATTERNS
        else:
            fname    = (f"multi_scale_penetration(1)_EVratio(0)_"
                        f"Concurrent_{weight_str}_{ped}_seed({seed}).txt")
            patterns = METRIC_PATTERNS
        path = os.path.join(RESULTS_DIR, fname)
        if not os.path.exists(path):
            continue
        for m, v in parse_file(path, patterns).items():
            if v is not None:
                vals[m].append(v)
    return vals
...
def significance_stars(base_arr, alt_arr):
    if len(base_arr) < 3:
        return ""
    _, t_p = stats.ttest_rel(base_arr, alt_arr)  # paired t-test
    try:
        _, w_p = stats.wilcoxon(np.array(alt_arr) - np.array(base_arr))
    except ValueError:
        w_p = float("nan")
...
def build_ped_rows(ped):
    act_sv  = load_seed_vals(ped, None, is_actuated=True)
    svcc_sv = load_seed_vals(ped, SVCC_WEIGHT)
    ...
    for mi, metric in enumerate(METRIC_ORDER):
        ...
        # ── SVCC (reference — show only raw value, no percentage, no stars) ──
        rows.append({
            "metric": metric, "metric_order": mi,
        ...
        # ── M@SVCC weights (value** + pct vs SVCC) ───────────────────────────
        for ci, w in enumerate(WEIGHTS_M2):
            wsv = load_seed_vals(ped, w)
            wm  = {m2: (round(float(np.mean(v)), 2) if v else None) for m2, v in wsv.items()}
            wv  = wm[metric]

            if sv and wv and svcc_sv[metric] and wsv[metric]:
                n     = min(len(svcc_sv[metric]), len(wsv[metric]))
                stars = significance_stars(svcc_sv[metric][:n], wsv[metric][:n])
...
def make_heatmap_section(rows, color_scale, show_x_labels, include_legend):
    ...
    # Layer 3a: normal-weight text (no stars)
    ...
    # Layer 3b: bold text (one or two stars)
    txt_bold = (
        base.mark_text(align="center", baseline="middle", fontSize=12,
...
def make_ped_label_placeholder(height):
    """Minimal placeholder on the right (no longer used for labels)."""
    data = [{"x": 0}]
    return (
...
def make_left_placeholder(height):
    """Invisible placeholder that reserves space on the left for category labels."""
    data = [{"x": 0}]
    return (
        alt.Chart(alt.Data(values=data))
        .mark_point(opacity=0)
...
def add_colorbar_pil(png_path, max_pct, scale=2):
    """
    Draw a horizontal gradient color bar + tick labels below the figure using PIL.
    Color stops match COLOR_RANGE at proportional positions on [-max_pct, +max_pct].
    """
    from PIL import Image, ImageDraw, ImageFont

    # Parse hex stops into RGB tuples
    stops_hex = COLOR_RANGE          # 5 colours
    stops_pos = [0.0, 0.25, 0.5, 0.75, 1.0]   # positions in [0,1]
    stops_rgb = [
    ...
    # Layout (all in final-image pixels, already at `scale` factor)
    bar_margin_l  = int(120 * scale)  # wider margins to shorten the bar
    ...
    bar_offset    = int(35 * scale)   # extra gap to avoid overlapping ped label
    ...
    for xi in range(bar_w):
        t = xi / (bar_w - 1)          # 0..1 across the bar
        ...
        draw.line([(bar_margin_l + xi, bar_top),
                   (bar_margin_l + xi, bar_top + bar_h - 1)],
                  fill=color + (255,))
...
def build_header_rows():
    # Place label at the left of the two center weight columns; dx shifts it right
    # to the true midpoint between them (each column = CHART_WIDTH / len(COL_SORT) wide)
...
def make_chart():
    ...
    # Color scale: 5-stop RdBu-style, domain derived from actual data (like original)
    all_pcts = [r["pct_for_color"] for rows in ped_rows.values()
                for r in rows if not r["is_actuated"] and not r["is_svcc"]]
    max_pct = max(abs(p) for p in all_pcts) if all_pcts else 30.0
    ...
    # ── M@SVCC header strip (above the first section only) ───────────────────
    hdr_main = (
        ...
        .mark_text(align="center", baseline="middle",
                   ...
                   dx=CHART_WIDTH / len(COL_SORT) * 1.5)   # shift right to centre over weight columns
        .encode(
    ...
    # Placeholders to align category (left) and ped-label (right) columns
    hdr_left = (
    ...
    # ── Per-ped-level sections (category label LEFT, ped label RIGHT) ─────────
    sections = []
    ...
    chart = (
        ...
        .properties(
            title=alt.TitleParams(
                text="Concurrent-Asymmetric: Performance Heatmap vs SVCC (1.0, 0.0)",
                subtitle=[
                    "Means across seeds 10, 1000, 10000, 23423, 30000",
                    "Blue = improvement over SVCC  |  Orange/Red = regression vs SVCC  |  Grey = Actuated (reference)",
                    "* p < 0.05  |  ** p < 0.01  (paired t-test & Wilcoxon signed-rank vs SVCC)",
                ],
...
def detect_section_boundaries(img_arr, col_w):
    """
    Detect y-start/end of each ped-level section by scanning for the
    placeholder column (rightmost col_w pixels) which is white in spacing rows
    and dark-grey in label areas.  Returns list of (y_start, y_end) tuples.
    """
    import numpy as np
    right_strip = img_arr[:, -col_w:, :3].mean(axis=(1, 2))
    # Label column placeholder is white (>240); actual data area is also mostly
    # white.  Find the SPACING ROWS between sections (very white rows in the
    # MIDDLE columns too).
    mid = img_arr[:, 100:-100, :3].mean(axis=(1, 2))
    in_section = False
    sections = []
    y_start = 0
    for y, (m, r) in enumerate(zip(mid, right_strip)):
        if not in_section and m < 245:   # first non-white row = section start
            y_start = y
            in_section = True
...
def add_category_labels_pil(png_path, scale=2):
    """
    Draw Sustainability / Mobility / Safety labels horizontally in the left
    placeholder column, centered on each group's row-span.
    Separator lines extend full-width across the figure at each group boundary.
    """
    from PIL import Image, ImageDraw, ImageFont
    import numpy as np
    ...
    for sec_start, sec_end in data_sections:
        ...
        for label, row_start, row_end in CATEGORIES:
            ...
            # Adaptive font: largest that fits horizontally in the column
            font = None
            ...
            # Draw text centered horizontally and vertically in its span
        ...
        # Separator lines at group boundaries — span only the table area
    ...
    # ── Vertical line between category column and the rest ───────────────────
    ...
    # ── "Metric" header above the y-axis label area ──────────────────────────
    # Scan horizontally just inside the first section top to find where
    # the leftmost non-white column starts (= start of the heatmap cells).
    scan_y = first_sec_start + int(2 * scale)
    cell_x_start = w  # fallback
    for xi in range(left_col_w + 1, w - right_col_w):
        r, g, b = arr[scan_y, xi, :3]
        if r < 240 or g < 240 or b < 240:   # non-white pixel → first cell col
            cell_x_start = xi
            break

    # "Metric" label sits above the first section, centred in the y-axis area
    metric_label = "Metric"
    metric_font  = None
    ...
    tw, th = bb[2] - bb[0], bb[3] - bb[1]
    # Place just above the first section
...
if __name__ == "__main__":
    print("Parsing Concurrent-Asymmetric results …")
    chart, max_pct = make_chart()

    out_html = os.path.join(OUT_DIR, "fig_asymmetric_altair_new.html")
    out_png  = os.path.join(OUT_DIR, "fig_asymmetric_altair_new.png")
    ...
    # Add category labels on the left via PIL
    add_category_labels_pil(out_png, scale=2)
    # Add color bar at the bottom via PIL
    add_colorbar_pil(out_png, max_pct, scale=2)
    # Add ped-level labels below each section (after colorbar so they render on top)
