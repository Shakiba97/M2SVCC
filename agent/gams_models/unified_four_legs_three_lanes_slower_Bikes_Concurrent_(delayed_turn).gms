...
parameter
	 ...
	 vehicle_length(i,j) 'length of vehicle (bike or passenger car)',
	 minGap(i,j) 'minimum gap between current and leading (bike=0.5, car=2)',
	 ...
	 bike_Demand(n,k) 'bike demand for each bike passing n for the next k steps of prediction horizon',
         Gp 'steps needed for extension',
	 Wv 'Weighting factor for Vehicles',
	 Wp 'Weighting factor for Pedestrians',
	 Wb 'Weighting factor for Bikes';

*$gdxin _gams_py_gdb0
$if not set gdxincname $abort 'no include file name for data file provided'
$gdxin %gdxincname%
$load i j l k vehicle_indi m n h d_0 k_dynamics lane_length vehicle_length minGap veh_dynamics gamma s_init wt_init p_init ped_Demand bike_Demand Gp Wv Wp Wb
$gdxin
...
binary variable
	 ...
	 x(n,k) 'whether bike pass n has the right of way';
...
vehicle_dynamics_2(i,j,k)$(vehicle_indi(i,j) and k_dynamics(k) ne 0)..           s(i,j,k) - s(i,j,k-1) =l= 26*h*(1-sum(l, p_c(l,k))) + 13*3*sum(l, p_c(l,k));
...
car_following(i,j,k)$(vehicle_indi(i,j) and veh_dynamics(i,j) ne 0)..            s(i,j,k) - s(i+1,j,k) =g= vehicle_length(i,j) + minGap(i+1,j)-0.1;
*car_following(i,j,k)$(vehicle_indi(i,j) and veh_dynamics(i,j) ne 0)..            s(i,j,k) - s(i+1,j,k) =g= 3;
...
ped_phase_equal_4(k)..                                                           p('5',k) + p('7',k) + p('9',k) =e= q('1',k);
ped_phase_equal_5(k)..                                                           p('9',k) =e= q('2',k);
ped_phase_equal_6(k)..                                                           p('9',k) =e= q('4',k);
bike_phase_equal_1(k)..								 p('1',k) + p('2',k) =e= x('7',k);
bike_phase_equal_2(k)..								 p('5',k) + p('6',k) =e= x('1',k);
bike_phase_equal_3(k)..								 p('1',k) + p('3',k) =e= x('3',k);
bike_phase_equal_4(k)..								 p('5',k) + p('7',k) =e= x('5',k);
bike_phase_equal_5(k)..								 x('1',k) =e= x('2',k);
bike_phase_equal_6(k)..								 x('3',k) =e= x('4',k);
bike_phase_equal_7(k)..								 x('5',k) =e= x('6',k);
bike_phase_equal_8(k)..								 x('7',k) =e= x('8',k);
