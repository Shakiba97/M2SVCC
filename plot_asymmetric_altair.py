"""
Altair (Vega-Lite) heatmap table for Concurrent-Asymmetric results.
...
Outputs HTML (interactive) + PNG (static via vl-convert).
"""
...
DATA_DIR = os.path.join(BASE, "Results", "Concurrent-Asymmetric")
...
def make_altair_chart(df):
    ...
    mid_col     = m2svcc_cols[len(m2svcc_cols) // 2]
    ...
    chart = alt.vconcat(
        ...
        title=alt.Title(
            ...
            subtitle=[
                "Mean across seeds 10, 1000, 10000, 23423, 30000",
...
def save_chart(chart):
    out_html = os.path.join(BASE, "Results", "Concurrent figurs", "fig_asymmetric_altair.html")
    chart.save(out_html)
    print(f"Saved (interactive HTML): {out_html}")

    try:
        import vl_convert as vlc
        out_png = os.path.join(BASE, "Results", "Concurrent figurs", "fig_asymmetric_altair.png")
